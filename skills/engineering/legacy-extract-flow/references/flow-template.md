...
| `BR-LOAN-002` | Applicants with a delinquency flag receive DECLINED regardless of amount | `LoanEntryServlet.java:82` (trigger), `LNAPPRVL` paragraph `CHKDELQ` lines 320-340 | On ESB-LOAN-001 response processing | (none) | Yes (probe #1) *(example: remove this row)* |
